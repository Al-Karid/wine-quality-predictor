...
class DataAccessTests(TestCase):
    ...
    def test_build_train_test_sets(self):
        num_data = list(range(10))
        str_data =  list(string.ascii_uppercase)[:10]
        label_data = [0]*5 + [1]*5
        ...
        # Adding assertion
        self.assertTrue("train" in train_test_sets.keys())
        self.assertTrue("test" in train_test_sets.keys())
        self.assertEqual(train_test_sets["train"][0].shape[0],train_size*len(num_data))
